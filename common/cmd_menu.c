	...
	while (1)
	{
		...
		switch (c)
		{
			case '1':
			{
				...
				printf("Enter the GT2440 IP address:(xxx.xxx.xxx.xxx)\n");
				readline(NULL);
				strcpy(param_buf2,console_buffer);
				printf("Enter the Mask IP address:(xxx.xxx.xxx.xxx)\n");
				readline(NULL);
				strcpy(param_buf3,console_buffer);
				printf("Enter NFS directory:(eg: /opt/GTStudio/root_nfs)\n");
				readline(NULL);
				strcpy(param_buf4,console_buffer);
				sprintf(cmd_buf, "setenv bootargs console=ttySAC0 root=/dev/nfs nfsroot=%s:%s ip=%s:%s:%s:%s:GT2440.GTStudio.net:eth0:off", param_buf1, param_buf4, param_buf2, param_buf1, param_buf2, param_buf3);
				printf("bootargs: console=ttySAC0 root=/dev/nfs nfsroot=%s:%s ip=%s:%s:%s:%s:GT2440.GTStudio.net:eth0:off\n",param_buf1, param_buf4, param_buf2, param_buf1, param_buf2, param_buf3);
...
void main_menu_usage(void)
{
	...
	printf("[7] Download Program (uCOS-II or 2440_Test) to SDRAM and Run it\r\n");
	printf("[8] Boot the system\r\n");
	printf("[9] Format the Nand Flash\r\n");
	printf("[0] Set the boot parameters\r\n");
	printf("[a] Download User Program (eg: uCOS-II or 2440_Test) to Nand Flash \r\n");
